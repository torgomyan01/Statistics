...
function VsPage() {
  ...
  useEffect(() => {
    if (vs_names && countries.length) {
      const [country1, country2] = (vs_names as string).split("-vs-");
      const country1Data = countries.find(
        (country) =>
          createSlug(country.nicename.toLowerCase()) ===
          createSlug(country1.toLowerCase()),
      );
      const country2Data = countries.find(
        (country) =>
          createSlug(country.nicename.toLowerCase()) ===
          createSlug(country2.toLowerCase()),
      );

      console.log(country1Data, country2Data);

      if (country1Data && country2Data) {
        setSelectedIsoCodeOne(country1Data.iso3);
        setSelectedIsoCodeTwo(country2Data.iso3);
      }
    }
  }, [vs_names, countries]);
          ...
          <div className="bg-white dark:bg-gray-800 pb-0 sm:pb-8 p-4 sm:p-8 rounded-3xl shadow-2xl mb-12 border-t-4 border-indigo-600 dark:border-indigo-500 transition-colors duration-500">
            ...
            <div className="flex flex-col md:flex-row gap-8 justify-around">
              <Autocomplete
                ...
                selectedKey={selectedIsoCodeOne}
              >
              ...
              <Autocomplete
                ...
                radius="sm"
                onSelectionChange={(key) => setSelectedIsoCodeTwo(key)}
                selectedKey={selectedIsoCodeTwo}
              >
                {countries.map((country: ICountry) => (
                  <AutocompleteItem
